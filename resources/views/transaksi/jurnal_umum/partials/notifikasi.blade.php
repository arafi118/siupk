                        ...
                        <li class="list-group-item border-0 d-flex justify-content-between ps-0 mb-2 border-radius-lg">
                            <div class="d-flex align-items-center">
                                <button class="btn btn-icon-only btn-rounded btn-outline-danger mb-0 me-3 p-3 btn-sm d-flex align-items-center justify-content-center">
                                    <i class="fa fa-chevron-down">expand_more</i>
                                </button>
                                <div class="d-flex flex-column">
                                    <h6 class="mb-1 text-dark text-sm">
                                        <b>{{ $trx->rek_kredit->kode_akun }} - {{ $trx->rek_kredit->nama_akun }}</b>
                                    </h6>
                                    <span class="text-xs">{{ Tanggal::tglLatin($trx->tgl_transaksi) }}</span>
                                </div>
                            </div>
                            <div class="d-flex align-items-center text-danger text-gradient text-sm font-weight-bold">
                                <b>- Rp. {{ number_format($trx->jumlah, 2) }}</b>
                            </div>
                        </li>
                        <li class="list-group-item border-0 d-flex justify-content-between ps-0 mb-2 border-radius-lg">
                            <div class="d-flex align-items-center">
                                <button class="btn btn-icon-only btn-rounded btn-outline-success mb-0 me-3 p-3 btn-sm d-flex align-items-center justify-content-center">
                                    <i class="fa fa-chevron-up">expand_less</i>
                                </button>
                                <div class="d-flex flex-column">
                                    <h6 class="mb-1 text-dark text-sm">
                                        <b>{{ $trx->rek_debit->kode_akun }} - {{ $trx->rek_debit->nama_akun }}</b>
                                    </h6>
                                    <span class="text-xs">{{ Tanggal::tglLatin($trx->tgl_transaksi) }}</span>
                                </div>
                            </div>
                    ...
                    <div class="d-flex justify-content-end" style="gap: 1rem">
                        @if ($kuitansi)
                            <button type="button" data-action="/transaksi/dokumen/kuitansi/{{ $trx->idt }}"
                                class="btn btn-sm btn btn-primary btn-link">Kuitansi</button>
                        @endif
                        <button type="button" data-action="/transaksi/dokumen/{{ $files }}/{{ $trx->idt }}"
                            class="btn btn-sm btn btn-info btn-link">{{ $files }}</button>
                    </div>
                </div>
            </div>
        </div>
